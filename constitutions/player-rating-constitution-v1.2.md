...
Player Rating Constitution v1.2

Canonical document. Supersedes Player Rating Constitution v1.1. This version publishes the deterministic veteran Ability adjustment already used by the rating pipeline so every final published Ability remains independently reproducible.
...
Ability represents current footballing level. Scale 1–100. It begins with current market value via the published Ability Curve. For qualifying veteran players only, the published Veteran Reality Adjustment below is then applied to prevent extreme age and weaker current competition from producing an implausibly high final current-Ability rating. No other post-model Ability adjustment is permitted unless first added to governance.
...
AbilityBase = A_low + (A_high − A_low) × (ln V − ln V_low) ÷ (ln V_high − ln V_low)
...
### Veteran Reality Adjustment

The Veteran Reality Adjustment is deterministic and applies only after the market-value Ability Curve has produced `AbilityBase`.

A player is reviewed only when:

- age is **34 or older**; and
- `AbilityBase` is **87 or higher**.

For this rule, the current competition is treated as **elite** when its canonical source competition code is one of:

- `GB1` — English Premier League;
- `ES1` — Spanish top division;
...
`LowValue = current Transfermarkt market value < €5,000,000`.

The first matching penalty is applied:

Condition| Ability penalty
Age 34–36, non-elite competition, LowValue| −1
Age 37–39, elite competition| −1
Age 37–39, non-elite competition| −2
Age 40+, elite competition, goalkeeper| −2
Age 40+, elite competition, outfield| −3
Age 40+, non-elite competition, goalkeeper| −3
Age 40+, non-elite competition, outfield| −4
Otherwise| 0

Final published Ability:

`Ability = clamp(round(AbilityBase − VeteranPenalty), 60, 99)`

There are no secret player-specific exemptions. Any future exception, threshold, league classification or penalty change must be published and version-controlled before it can affect canonical ratings.

The purpose is not to reward or punish age by itself. It is a current-reality safeguard for the narrow high-rated veteran tail where historic market reputation can otherwise lag substantially behind present competitive context.

Worked examples:

- Age 38, AbilityBase 90, elite competition → penalty 1 → final Ability 89.
- Age 38, AbilityBase 90, non-elite competition → penalty 2 → final Ability 88.
- Age 41 outfield player, AbilityBase 89, non-elite competition → penalty 4 → final Ability 85.
- Age 35, AbilityBase 88, non-elite competition, market value €4m → penalty 1 → final Ability 87.
- Age 35, AbilityBase 88, elite competition → no veteran penalty → final Ability 88.
...
The published Veteran Reality Adjustment is part of the formula, not an administrator override.
...
New Database Entrants — initial Ability derived from market value via the Ability Curve and, where applicable, the published Veteran Reality Adjustment; Potential generated immediately under the Potential Regime.
